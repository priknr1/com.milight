...
module.exports.init = function (devices_data, callback) {
	...
	Homey.manager('flow').on('action.white_mode', function (callback, args) {
		...
		if (args.device) {
			...
			var device = getDevice(args.device.id);
			if (device) {

				device.zone.enableWhiteMode();

				// Give realtime update about current mode
...
module.exports.capabilities = {
	...
	light_mode: {
		get: function (device_data, callback) {
			...
			var device = getDevice(device_data.id);
			if (device) {
				callback(null, device.zone.light_mode);
			}
			else {
				callback(true, null);
			}
		...
		set: function (device_data, light_mode, callback) {
			...
			if (device) {
				...
				if (light_mode == "temperature") {
					...
					// Activate color mode
					device.zone.enableColorMode();

					// Give realtime update about current hue
					module.exports.realtime(device_data, 'light_hue', device.zone.hue);
