...
// TODO refactor this.light and _sendCommand for v6 and legacy
const DEBUG = true;
const TYPE = {
	RGBW: 'RGBW',
	RGB: 'RGB',
...
class Zone {
	 ...
	 */
	setHue(hue) {
		if (!hue) return new Error('missing_hue_parameter');
		...
		if (this.type == TYPE.RGBW || this.type == TYPE.RGB || this.type === TYPE.BRIDGE) {
			...
			_log(`setHue() -> hue: ${hue} -> zone:${this.number}_${this.type} to ${hue} (light:${!!this.light})`);
