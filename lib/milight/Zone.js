...
class Zone {
	...
	async setSaturation(saturation) {
		...
		_log(`setSaturation() -> saturation: ${saturation} -> zone:${this.number}_${this.type} to ${saturation}`);

		// Only available on rgbww
		if (this.type === TYPE.RGBWW && this.bridgeVersion === 6) {
			this.saturation = saturation;
			return this._sendCommand([
				this.commands.on(this.number),
				this.commands.saturation(this.number, Math.round(map(0, 1, 100, 0, saturation)))
			]);
		}
