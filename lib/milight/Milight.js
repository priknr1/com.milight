...
class Milight {
	...
	/**
	 * Construct Milight object, directly start polling for bridge availability.
	 */
	constructor() {
	 ...
	 * @private
	 */
	static _discoverBridges(options) {
		...
		return discoverBridges(options);
	}
	...
	getBridge(query) {
		...
		const bridge = this.bridges.find(obj => {
			return obj.mac === mac && (!available || (available && obj.unavailableCounter === 0));
		});
	...
	registerBridge(bridge, temp) {
		...
		if (!bridge.name && !bridge.bridgeVersion) return new Error('missing_bridge_name_or_version');
		...
		if (this.hasBridge(bridge)) {
			...
			_log('registerBridge() -> bridge was already registered, updating it');

			// Then update the bridge currently in memory
			const registeredBridge = this.updateBridge(this.getBridge(bridge), bridge);
	...
	discoverBridges(options) {
		...
		return Milight._discoverBridges(options)
			.then(bridges => {
	...
	findBridge(mac) {
		return new Promise((resolve, reject) => {
			...
			const bridge = this.getBridge({ mac: mac, available: true });
