...
class Bridge extends EventEmitter {
	...
	constructor(options) {
		...
		// If bridgeVersion is 6 use NodeMilightPromise
		// TODO refactor legacy bridge to use NodeMilightPromise as well
		if (this.bridgeVersion === 6) {
			this.light = new NodeMilightPromise({
				ip: this.ip,
				type: 'v6',
			});
		}
	...
	updateIPAddress(ip) {
		...
		if (this.bridgeVersion === 6) {
			...
			// Create new NodeMilightPromise object to re-initiate a session with the bridge
			this.light = new NodeMilightPromise({
				ip: this.ip,
				type: 'v6',
			});
